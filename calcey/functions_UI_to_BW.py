...
def country_from_coordinates(Latitude: float, Longitude: float):
...
def ISO3_country_from_coordinates(Latitude: float,Longitude:float):
...
def country_fertilizer_proxy_location(Latitude: float, Longitude: float):
    country_ISO3 = ISO3_country_from_coordinates(Latitude, Longitude)
...
def get_background_data_fert(Latitude: float, Longitude: float, crop:str):
    # get fetilizer data
    ...
    crop_mapping_file = get_crop_mapping_file(sheet_name="Mapping_Fertilizer")
    
    # get country proxy in ISO 3 format
    country_proxy_ISO3 = country_fertilizer_proxy_location(Latitude, Longitude)
    ...
    try:
        ...
        for level in range(1,len(crop_mapping_file.columns)-1):
            try:
                crop_proxy_value = crop_proxy_excelwithsheet(crop=crop,sheet_name="Mapping_Fertilizer",level=level)
                return(df_amount_fert_crop_country.loc[(country_proxy_ISO3,crop_proxy_value)])
            except KeyError:
                pass


def get_background_data_yield(input_lat: float, input_lon: float, crop: str):
    # get yield data
    df = pd.read_csv("../data/FAOSTAT_data_yield.csv")
    country = country_from_coordinates(Latitude=input_lat, Longitude=input_lon)
    FAO_yield = float(df.loc[(df['Area'] == country) & (df['Year'] == 2022) & (df['Item'] == crop), 'Value'].iloc[0])
    return FAO_yield


def get_background_data_water(input_lat: float, input_lon: float, crop: str):
    path_to_folder = "../data/NC_background_water/"

    crop_water = crop_proxy_excelwithsheet(crop=crop, sheet_name="Mapping_water")
...
def get_background_data_pest(input_lat: float, input_lon: float, crop: str):
    path_to_folder = "../data/NC_background_pest/"

    crop_pest = crop_proxy_excelwithsheet(crop=crop, sheet_name="Mapping_pesticides")
